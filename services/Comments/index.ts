...
class CommentServer {
  ...
  async initRoutes() {
    ...
    this.app.post("/addComment", async (req: Request, res: Response) => {
      console.log("RECIEVED ADD COMMENT QUERY", req.query);
      ...
      const comment = await self.db.createComment(
        username,
        userId,
        parentId,
        commentId,
  ...
  async sortByVotes(commentList: any[]) {
    ...
    for (let i = 0; i < commentList.length; i++) {
      ...
      if (voteObj.data.length === 0) {
        ...
        let pushed = false;
        let votes =
          voteObj.data[0].upvotes.length + voteObj.data[0].downvotes.length;
