...
class CommentVoteServer{
  ...
  async initRoutes(){
    ...
    this.app.post('/comments/vote', async (req: Request,res: Response) => {
      ...
      if(storedVotes.length === 0 ){ //if it's the first vote on this comment
        let upvotes:string[] = []
        ...
        const voteObj = await self.db.createCommentVote(commentId, upvotes, downvotes, ownerId)
        ...
        const voteObj = await self.db.updateVote(commentId, newUpvotes, newDownvotes);
        console.log(voteObj)
        res.status(200).send(JSON.stringify(voteObj)) //voteObj not sending right object? need to debug
      }
